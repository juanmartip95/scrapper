...
(async () => {
    try {
        ...
        await page.goto('');//URL here eza

        const scrollDistance = 33000; // Adjust the scroll distance as needed
        const scrollInterval = 3600; // Adjust the interval between scrolls as needed
        const maxScrollAttempts = 60; // Maximum number of scroll attempts
        ...
        const photoUrls = await page.$$eval('.thumb_img', (images) =>
            images.map((image) => ({
                url: image.src,
